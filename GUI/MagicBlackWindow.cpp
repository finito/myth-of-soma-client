...
MagicBlackWindow::MagicBlackWindow()
{
	try
	{
		window_ = WindowManager::getSingleton().loadWindowLayout("magicblackwindow.layout");
		window_->hide();

		for (int i = 0; i < MAGIC_BLACK_SLOT_COUNT; ++i)
		{
			iconSlots_[i] = window_->getChild("MagicBlack/Icon" + PropertyHelper::intToString(i+1));
			iconSlots_[i]->setDragDropTarget(false);
			iconSlots_[i]->subscribeEvent(Window::EventMouseEntersArea,
				Event::Subscriber(&MagicBlackWindow::HandleMouseEnterIconArea, this));

			descriptionSlots_[i] = window_->getChild("MagicBlack/Description" + PropertyHelper::intToString(i+1));
		}

		scrollBar_ = static_cast<Scrollbar*>(window_->getChild("MagicBlack/Scrollbar"));

		closeButton_ = static_cast<PushButton*>(window_->getChild("MagicBlack/CloseButton"));

		auraButton_ = static_cast<PushButton*>(window_->getChild("MagicBlack/AuraButton"));
		blueButton_ = static_cast<PushButton*>(window_->getChild("MagicBlack/BlueButton"));
		whiteButton_ = static_cast<PushButton*>(window_->getChild("MagicBlack/WhiteButton"));
		abilityButton_ = static_cast<PushButton*>(window_->getChild("MagicBlack/AbilityButton"));

		window_->subscribeEvent(Window::EventShown,
			Event::Subscriber(&MagicBlackWindow::HandleShown, this));

		window_->subscribeEvent(Window::EventMouseWheel,
			Event::Subscriber(&MagicBlackWindow::HandleScroll, this));

		closeButton_->subscribeEvent(PushButton::EventClicked,
			Event::Subscriber(&MagicBlackWindow::HandleClose, this));

		auraButton_->subscribeEvent(PushButton::EventClicked,
			Event::Subscriber(&MagicBlackWindow::HandleAuraButton, this));

		blueButton_->subscribeEvent(PushButton::EventClicked,
			Event::Subscriber(&MagicBlackWindow::HandleBlueButton, this));

		whiteButton_->subscribeEvent(PushButton::EventClicked,
			Event::Subscriber(&MagicBlackWindow::HandleWhiteButton, this));

		abilityButton_->subscribeEvent(PushButton::EventClicked,
			Event::Subscriber(&MagicBlackWindow::HandleAbilityButton, this));

		scrollBar_->setStepSize(1.0f);
		scrollBar_->setPageSize(static_cast<float>(MAGIC_BLACK_SLOT_COUNT));
		scrollBar_->setOverlapSize(0);
		scrollBar_->setDocumentSize(0);

		scrollBar_->subscribeEvent(Scrollbar::EventScrollPositionChanged,
			Event::Subscriber(&MagicBlackWindow::HandleScrollPositionChanged, this));

		dragItem_ = WindowManager::getSingleton().createWindow("Soma/StaticImage", "MagicBlack/DragItem");
		...
		dragContainer_ = static_cast<DragContainer*>(
			WindowManager::getSingleton().createWindow("DragContainer", "MagicBlack/DragContainer"));
		...
		dragContainer_->subscribeEvent(DragContainer::EventDragStarted,
			Event::Subscriber(&MagicBlackWindow::HandleDragStarted, this));
		dragContainer_->subscribeEvent(DragContainer::EventDragEnded,
			Event::Subscriber(&MagicBlackWindow::HandleDragEnded, this));

		System::getSingleton().getGUISheet()->addChildWindow(window_);
	}
	catch (Exception& e)
	{
		std::cerr << "MagicBlackWindow: Failed to initialize: " << e.what() << std::endl;
	}
}
...
bool MagicBlackWindow::HandleClose(const EventArgs& e)
{
	window_->hide();
	return true;
}

bool MagicBlackWindow::HandleShown(const EventArgs& e)
{
...
void MagicBlackWindow::Update()
{
	...
	const float documentSize   = stepSize * std::ceilf(inventoryComponent->GetBlackMagicSize() / stepSize);
	...
	for (int i = 0; i < MAGIC_BLACK_SLOT_COUNT; ++i)
	{
		...
		if (index >= inventoryComponent->GetBlackMagicSize())
		{
			break;
		}

		const MagicDataPtr magic = inventoryComponent->GetBlackMagic(index);
...
bool MagicBlackWindow::HandleScrollPositionChanged(const EventArgs& e)
{
	Update();
	return true;
}

bool MagicBlackWindow::HandleScroll(const EventArgs& e)
{
...
bool MagicBlackWindow::HandleAuraButton(const EventArgs& e)
{
	window_->hide();
...
bool MagicBlackWindow::HandleBlueButton(const EventArgs& e)
{
	window_->hide();
...
bool MagicBlackWindow::HandleWhiteButton(const EventArgs& e)
{
	window_->hide();
...
bool MagicBlackWindow::HandleAbilityButton(const EventArgs& e)
{
	window_->hide();
...
bool MagicBlackWindow::HandleDragStarted(const EventArgs& e)
{
...
bool MagicBlackWindow::HandleDragEnded(const EventArgs& e)
{
...
bool MagicBlackWindow::HandleMouseEnterIconArea(const EventArgs& e)
{
	const MouseEventArgs& mouseEventArgs = static_cast<const MouseEventArgs&>(e);
...
void MagicBlackWindow::UpdateDragContainer(Window& window)
{
